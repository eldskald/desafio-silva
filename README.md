...
O desafio foi feito com javascript puro, usando TailwindCSS e Jest como bibliotecas apenas. O foco foi em simplicidade, com o mínimo possível de dependências para um processo mais fácil possível para desenvolver e fazer o deploy, que nesse caso foi feito pelo GitHub Pages, que pode ser acessado em [eldskald.github.io/desafio-silva](https://eldskald.github.io/desafio-silva/).
...
O deploy está feito com GitHub Pages, usando GitHub Actions para automatizar atualizações no deploy automaticamente, também para checar se os testes ou linter estão falhando. Como é um projeto pequeno só comigo, resolvi não proteger a main e fazer pushes diretos nela, só por facilidade mesmo. Se alguma action falhar eu reverto o commit, mais fácil do que ficar fazendo PR pra rodar os testes e linter antes de entrar na main.

Porque escolhi GitHub Pages, eu não consigo usar um db para permanência dos dados. Porém, eu escrevi os scripts das "interações" com os dados de um jeito que trocar pra requests http puras fique fácil.
